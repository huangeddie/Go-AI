...
def worker_play(queue, args1, args2, worker_episodes):
    ...
    timestart = time.time()
    p1wr, black_wr, steps, replay_mem = game.play_games(go_env, pi1, pi2, worker_episodes)
    timeend = time.time()

    duration = timeend - timestart
    total_steps = sum(steps)

    queue.put((p1wr, black_wr, duration, total_steps))
...
def multi_proc_play(args1, args2, requested_episodes, workers=8):
    ...
    for _ in range(workers):
        p = context.Process(target=worker_play, args=(queue, args1, args2, worker_episodes))
        p.start()
        processes.append(p)
    ...
    while not queue.empty():
        p1wr, black_wr, duration, steps = queue.get()
        p1wrs.append(p1wr)
        black_wrs.append(black_wr)
        total_durations.append(duration)
        total_steps.append(steps)

    assert len(p1wrs) == workers, len(p1wrs)
